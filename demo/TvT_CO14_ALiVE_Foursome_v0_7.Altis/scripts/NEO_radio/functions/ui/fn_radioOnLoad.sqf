...
//Available Supports
if (count _transportArray > 0 && _action != "talkarty") then { _available set [count _available, ["TRANSPORT", "\a3\Ui_f\data\GUI\Cfg\CommunicationMenu\transport_ca.paa"]] };
if (count _casArray > 0 && _action == "radio") then { _available set [count _available, ["CAS", "\a3\Ui_f\data\GUI\Cfg\CommunicationMenu\cas_ca.paa"]] };
...
switch (_action) do
{
	...
	case "talkarty" :
	{
		...
		[] spawn
		{
			lbSetCurSel [_suppListBox, 2];
			waituntil { lbSize _artylist > 0 };
			lbSetCurSel [_artylist, 0];
		};
	};
	...
	case "radio" :
	{
		...
		{
			[] spawn { lbSetCurSel [_suppListBox, 0] };
		}
		else
		{
