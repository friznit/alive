...
if (isNil "_respawnObject") then {
	_mySoldier setPos ([myRespawnPoint, 1, 25, 1, 0, 5, 0,[],[myRespawnPoint]] call BIS_fnc_findSafePos); // he hasn't
} else {
	//Check if the HQ he signed in at is still 'alive'
	if (alive _respawnObject) then {
        _positioner = ([position _respawnObject, 1, 25, 1, 0, 5, 0,[],[position _respawnObject]] call BIS_fnc_findSafePos);
        ...
        _mySoldier setPos ([myRespawnPoint, 1, 25, 1, 0, 5, 0,[],[myRespawnPoint]] call BIS_fnc_findSafePos);
	};
};