private ["_display", "_map", "_transportUnitLb", "_transportTaskLb", "_transportConfirmButton", "_slider", "_transportFlyHeightSlider","_transportRoeCombo","_ROE"];
...
_ROE = _transportRoeCombo lbData (lbCurSel _transportRoeCombo);

if(isNil "_ROE") then {_ROE = "BLUE"};
//New Task Assigned
private ["_arguments"];
_arguments = [_task, _pos, _ROE];

if (toUpper _task == "CIRCLE" || toUpper _task == "INSERTION") then
{
	_arguments = [_task, _pos, _ROE,_amnt];
};
