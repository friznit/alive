...
if( _status == "NONE" || _status == "" ) then { _status = _chopper getVariable ["NEO_radioCasUnitStatus","NONE"]; };
diag_log format["%1 %2 %3",time,_chopper,_transportArray];
...
if (_status != "KILLED") then
{
	...
	{
		_transportRoeCombo lbAdd (_x select 0);
		_transportRoeCombo lbSetData [_foreachindex, (_x select 1)];
	} forEach [["Roe - Hold Fire","BLUE"], ["Roe - Fire","YELLOW"]];
