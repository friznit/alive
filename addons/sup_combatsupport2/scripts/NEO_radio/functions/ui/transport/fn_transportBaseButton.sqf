...
_callsign = groupId (_transportArray select (lbCurSel _transportUnitLb) select 1); if (!isNil { NEO_radioLogic getVariable "NEO_radioTalkWithPilot" }) then { _callsign = (format ["%1", _grp]) call NEO_fnc_callsignFix };
