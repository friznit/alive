...
if( _status == "NONE" || _status == "" ) then { _status = _veh getVariable["NEO_radioTrasportUnitStatus","NONE"]; };
diag_log format["%1 %2 %3",time,_veh,_casArray];
...
_casHelpUnitText ctrlSetStructuredText parseText (switch (_status) do
{
	...
	default { "<t color='#627057' size='0.7' font='PuristaMedium'>Unit is available and waiting for task</t>" };
});
