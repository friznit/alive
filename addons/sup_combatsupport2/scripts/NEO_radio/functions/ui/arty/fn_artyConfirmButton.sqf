...
_ord = ((_battery getVariable "NEO_radioArtyBatteryRounds") select (lbCurSel _artyOrdnanceTypeLb)) select 0;
...
_callsign = groupId (_artyArray select (lbCurSel _artyUnitLb) select 1); if (!isNil { NEO_radioLogic getVariable "NEO_radioTalkWithArty" }) then { _callsign = ((NEO_radioLogic getVariable "NEO_radioTalkWithArty") getVariable "NEO_radioArtyModule") select 1 };
_callsignPlayer = (format ["%1", group player]) call NEO_fnc_callsignFix;

//Dislog from player
[[player,format["%1, this is %2. We need an %5 %7 round %6 strike at grid %3%4 with %8m dispersion and %9s delay. Over.", _callsign, _callSignPlayer, _coord select 0, _coord select 1, _type, _ordnanceType, _count, _dispersion, _rate],"side"],"NEO_fnc_messageBroadcast",true,true] spawn BIS_fnc_MP;
