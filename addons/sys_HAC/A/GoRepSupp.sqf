...
_logic setvariable ["HAC_HQ_RepPoints",(_logic getvariable "HAC_HQ_RepPoints") + [_Trg]];
...
if ((_logic getvariable "HAC_HQ_Debug") or (isPlayer (leader _unitG))) then 
	{
	_i = [[_posX,_posY],_unitG,"markRepSupp","ColorKhaki","ICON","End"," Repair A"," - REPAIR SUPPORT",[0.6,0.6],180,_logic] call ALiVE_fnc_HAC_Mark
	};
...
while {(_counter <= 3)} do
	...
	if not (_alive) exitwith {if ((_logic getvariable "HAC_HQ_Debug") or (isPlayer (leader _unitG))) then {deleteMarker ("markRepSupp" + str (_unitG))};_logic setvariable ["HAC_HQ_RepPoints",(_logic getvariable "HAC_HQ_RepPoints") - [_Trg]];;_unitG setVariable [("Busy" + _unitvar), false];};
...
if (((damage _Trg) < 0.1) or ((damage _Trg) >= 0.9) or (isNull (group (assigneddriver (_this select 1))))) then {_damaged = _damaged - [_Trg]};
if ((_logic getvariable "HAC_HQ_Debug") or (isPlayer (leader _unitG))) then {deleteMarker ("markRepSupp" + str (_unitG))};
_lastOne = true;
...
if (_lastOne) then {_logic setvariable ["HAC_HQ_RSupportedG",(_logic getvariable "HAC_HQ_RSupportedG") - [group _Trg]]};
