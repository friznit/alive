...
_Otherthreat = [];
	...
	if ((_x in (_logic getvariable "HAC_HQ_EnHArmor")) and not (_GE in _LArmorthreat) and not (_checked)) then {_LArmorthreat set [(count _LArmorthreat),_GE]};
	if ((_x in (_logic getvariable "HAC_HQ_EnLArmor")) and not (_GE in _reconthreat) and not (_checked)) then {_reconthreat set [(count _reconthreat),_GE]};
